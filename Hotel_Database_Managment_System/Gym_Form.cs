...
{
    public partial class Gym_Form : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = "INSERT INTO  `gym`( `RoomId`, `ReservationId`, `Hours`, `Date`, `Payment`) " +
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox1.Text.Length>0) { 
             string sql = "UPDATE `gym` SET `RoomId`='"+ textBox2.Text + "',`ReservationId`='"+ textBox3.Text + "',`Hours`='" + textBox4.Text + "'," +
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (textBox1.Text.Length > 0)
                {
                    string sql = "DELETE FROM `gym` WHERE GuestId = " + int.Parse(textBox1.Text.ToString()) + "";
                    MySqlCommand cmd = new MySqlCommand(sql, databaseConnection);
                    cmd.ExecuteNonQuery();
